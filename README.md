...
Self-hosted web app for downloading media from **YouTube, Instagram, TikTok, Vimeo, SoundCloud, Spotify, Deezer, JOOX, TIDAL, Apple Music, and Beatport** — including **playlists/albums and pasted URL lists as ZIP** — built as a study of modern media pipelines: stream selection, lossless remuxing, and metadata handling. One Docker container: FastAPI + [yt-dlp](https://github.com/yt-dlp/yt-dlp) + ffmpeg behind a clean, no-build-step web UI.
...
- **Instagram & TikTok.** Reels, posts, carousels (every video in a multi-item post, ZIPped), and TikTok videos download with clean, sortable filenames: `handle - date - first caption words - reel/post/video - permalink id.mp4`. Options are "Original" (bit-exact) and "MP4 (H.264)" (most compatible). Instagram may require instagram.com cookies in your cookies file when the server's IP is blocked from anonymous access; TikTok usually needs nothing.
- **Batch paste.** Paste a whole list of links into the same box — one per line, comma-separated, or straight from a spreadsheet column — and it becomes a checkbox picker with ZIP download. Platforms can be mixed in one batch; the common case is a list of Instagram reels.
